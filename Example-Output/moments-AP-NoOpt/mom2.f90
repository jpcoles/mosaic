...
! OPS  2*ADD + 2*DIV + 26*MUL + 3*NEG + 16*POW + 2*SUB = 51  (59 before optimization)
subroutine mom_es_M2L(M, r, L)
    real(dp), intent(in)  :: M(0:MOM_ES_M_END)
    real(dp), intent(in)  :: r(1:3)
    real(dp), intent(out) :: L(0:MOM_ES_L_END)
    real(dp) :: a1, a2, b1, b2, c1, c2, u2, u3, u4, u5, h, u, D_x, D_y, D_z,&
...
subroutine mom_es_M2L2(M1, r1, L1, M2,r2,L2)
    ...
    real(dp), dimension(1:2) :: a1, a2, b1, b2, c1, c2, u2, u3, u4, u5, h, u, D_x, D_y, D_z,&
                D_xx, D_xy, D_xz, D_yy, D_yz, D_zz
#define x                    r(1:2,1)
#define y                    r(1:2,2)
#define L_x                  L(1:2,0)
