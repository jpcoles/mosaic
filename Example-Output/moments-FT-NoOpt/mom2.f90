...
! OPS  2*ADD + 2*DIV + 11*MUL + 5*POW + 2*SUB = 22  (31 before optimization)
...
! OPS  20*ADD + 6*DIV + 29*MUL + NEG + 5*POW + 4*SUB = 65  (77 before optimization)
subroutine mom_es_M2M(M, r, Ms)
    ...
    real(dp) :: a1, a2, b1, b2, c1, c2, u2, u3, u4, u5, h, u, M_zz, S_0, S_x,&
                S_y, S_z, S_xx, S_xy, S_xz, S_yy, S_yz
#define x                    r(1)
#define y                    r(2)
#define z                    r(3)
#define Ms_0                 Ms(0)
...
#define L_yz                 L(8)
    ...
    L_yz = D_yz*M_0
    D_zz = -(D_xx + D_yy)
    L_0  = D_0*M_0 + D_x*M_x + D_xy*M_xy + D_xz*M_xz + D_y*M_y + D_yz*M_yz + D_z* &
      M_z + (1.0d0/2.0d0)*(D_xx*M_xx + D_yy*M_yy + D_zz*M_zz)
...
#define L_yz                 L(8)
    ...
    L_yz = L_yz + (D_yz*M_0)
    D_zz = -(D_xx + D_yy)
    L_0  = L_0 + (D_0*M_0 + D_x*M_x + D_xy*M_xy + D_xz*M_xz + D_y*M_y + D_yz*M_yz + D_z* &
      M_z + (1.0d0/2.0d0)*(D_xx*M_xx + D_yy*M_yy + D_zz*M_zz))
